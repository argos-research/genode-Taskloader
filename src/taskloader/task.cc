...
Task::Meta::Meta(const Task& task) :
	ram{},
	cpu{task.name().c_str(), (task._desc.priority)*(Genode::Cpu_session::PRIORITY_LIMIT >> Genode::log2(128)), task._desc.deadline*1000, Genode::Affinity(Genode::Affinity::Space(4,1), Genode::Affinity::Location(1,0))},
	rm{},
	pd{},
	server{ram}
{
...
Task::Task(Server::Entrypoint& ep, Genode::Cap_connection& cap, Shared_data& shared, const Genode::Xml_node& node, Sched_controller::Connection* ctrl) :
		...
		_schedulable(true)
{
	if(_desc.priority>127||_desc.priority<0)
	{
		_desc.priority=0;
	}
